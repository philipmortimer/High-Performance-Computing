...
#define TIMESTEPLOOP(ii, jj, tot_u, tot_cells) \
    do { \
...
static const float rec_c_sq = 1.f / (1.f / 3.f);
static const float rec_c_sq_2 = 1.f / (2.f * (1.f / 3.f));
static const float w1_div_2_c_sq_square = (1.f / 9.f) / (2.f * (1.f / 3.f) * (1.f / 3.f));
static const float w2_div_2_c_sq_square = (1.f / 36.f) / (2.f * (1.f / 3.f) * (1.f / 3.f));
static const float c_sq_squared_2 = 2.f * (1.f / 3.f) * (1.f / 3.f);
static const float c_sq_2 = 2.f * (1.f / 3.f);
static const float neg_c_sq = -(1.f / 3.f);
...
void compute_main_grid(const t_param params,
  ...
  const int* obstacles,
  float*restrict tot_u_ret, int*restrict tot_cells_ret, const t_grid_bounds bounds, const int tt);
...
float timestep_align(const t_param params,
  ...
  float*restrict tmp_cells_speed0,float*restrict tmp_cells_speed1,float*restrict tmp_cells_speed2,float*restrict tmp_cells_speed3,float*restrict tmp_cells_speed4,float*restrict tmp_cells_speed5,float*restrict tmp_cells_speed6,float*restrict tmp_cells_speed7,float*restrict tmp_cells_speed8,
  const int* obstacles,
  float* av_vel_ret, int* tot_cells_ret, const t_grid_bounds bounds, const int tt);
...
int main(int argc, char* argv[])
{
  ...
  for (int tt = 0; tt < params.maxIters; tt++)
  {
    timestep_align(params, 
      ...
      obstacles,
      &av_vels[tt], &tot_cells[tt], bounds, tt);
    // Swaps pointers
  ...
  float* av_vel_ret, int* tot_cells_ret, const t_grid_bounds bounds, const int tt)
{
  ...
  compute_main_grid(params,
    cells_speed0,cells_speed1,cells_speed2,cells_speed3,cells_speed4,cells_speed5,cells_speed6,cells_speed7,cells_speed8,
    tmp_cells_speed0,tmp_cells_speed1,tmp_cells_speed2,tmp_cells_speed3,tmp_cells_speed4,tmp_cells_speed5,tmp_cells_speed6,tmp_cells_speed7,tmp_cells_speed8,
    obstacles,
    &tot_u_mid, &tot_cells_mid, bounds, tt);
  //printf("A*");
  // Waits to receive all data
  MPI_Waitall(6, receive_requests,
                MPI_STATUSES_IGNORE);
  float tot_u_above = 0.f;
  int tot_cells_above = 0;
  ...
  for (int ii = 0; ii < params.nx; ii++)
  {
    TIMESTEPLOOP(ii, bounds.nrows, tot_u_above, tot_cells_above);
  }
  
  float tot_u_below = 0.f;
  int tot_cells_below = 0;
  if(bounds.nrows != 1) {
    #pragma omp simd aligned(cells_speed0:ALIGN,cells_speed1:ALIGN,cells_speed2:ALIGN,cells_speed3:ALIGN,cells_speed4:ALIGN,cells_speed5:ALIGN,cells_speed6:ALIGN,cells_speed7:ALIGN,cells_speed8:ALIGN,tmp_cells_speed0:ALIGN,tmp_cells_speed1:ALIGN,tmp_cells_speed2:ALIGN,tmp_cells_speed3:ALIGN,tmp_cells_speed4:ALIGN,tmp_cells_speed5:ALIGN,tmp_cells_speed6:ALIGN,tmp_cells_speed7:ALIGN,tmp_cells_speed8:ALIGN,obstacles:ALIGN)
    for (int ii = 0; ii < params.nx; ii++)
    {
      TIMESTEPLOOP(ii, 1, tot_u_below, tot_cells_below);
    }
  }

  float tot_u = tot_u_mid + tot_u_above + tot_u_below;
  int tot_cells = tot_cells_mid + tot_cells_above + tot_cells_below;
...
void compute_main_grid(const t_param params,
  ...
  int tot_cells_mid = 0;
  // Computes grid cells not impacted by Halo cells
  #pragma omp parallel for reduction(+: tot_u_mid, tot_cells_mid)
  for (int jj_idx = 2; jj_idx < bounds.nrows; jj_idx++)
  {
    #pragma omp simd reduction(+: tot_u_mid, tot_cells_mid) aligned(cells_speed0:ALIGN,cells_speed1:ALIGN,cells_speed2:ALIGN,cells_speed3:ALIGN,cells_speed4:ALIGN,cells_speed5:ALIGN,cells_speed6:ALIGN,cells_speed7:ALIGN,cells_speed8:ALIGN,tmp_cells_speed0:ALIGN,tmp_cells_speed1:ALIGN,tmp_cells_speed2:ALIGN,tmp_cells_speed3:ALIGN,tmp_cells_speed4:ALIGN,tmp_cells_speed5:ALIGN,tmp_cells_speed6:ALIGN,tmp_cells_speed7:ALIGN,tmp_cells_speed8:ALIGN,obstacles:ALIGN)
    for (int ii_idx = 0; ii_idx < params.nx; ii_idx++)
    {
      TIMESTEPLOOP(ii_idx, jj_idx, tot_u_mid, tot_cells_mid);
    }
  }
  *tot_u_ret = tot_u_mid;
  *tot_cells_ret = tot_cells_mid;
}
              ...
              int** tot_cells_ptr, t_grid_bounds bounds, float** global_av_vels_ptr, int**  global_tot_cells_ptr)
{
  ...
  *obstacles_ptr = _mm_malloc(sizeof(int) * (bounds.nrows * params->nx),ALIGN);
  ...
  // For effective NUMA acess, initialise data in parallel
  #pragma omp parallel for
  for(int jj = 2; jj < bounds.nrows; jj++) {
    INITCELLS(jj);
  }
  INITCELLS(1);
  INITCELLS(bounds.nrows);
